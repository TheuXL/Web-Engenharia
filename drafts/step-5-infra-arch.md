...
```mermaid
flowchart LR
  S[Sensor/Edge Device] -->|heartbeat JSON| E[Phoenix Endpoint<br/>POST /api/telemetry/ingest]
  E -->|cast rápido| I[GenServer<br/>WCore.Telemetry.Ingestor]
  I --> ETS[(ETS<br/>:w_core_telemetry_cache)]
  ETS -->|flush em lote (~5s)| W[Write-Behind Worker<br/>WCore.Telemetry.WriteBehindWorker]
  W -->|insert_all + conflict_target| DB[(SQLite<br/>w_core.db em /data)]
```
...
2. **`w_core/Dockerfile` multi-stage**
...
| Arquivo | Papel |
|----------|------|
| `w_core/Dockerfile` | build multi-stage + runtime enxuto |
| `w_core/rel/env.sh.eex` | defaults de runtime para Edge/container |
